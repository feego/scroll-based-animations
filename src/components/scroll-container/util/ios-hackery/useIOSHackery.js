 ...
 */
export const iDevices = ['iPad Simulator', 'iPhone Simulator', 'iPod Simulator', 'iPad', 'iPhone', 'iPod'];
...
export const isIOSDevice = () =>
  typeof window !== 'undefined' && !!window.navigator.platform && Boolean(iDevices.find((device) => navigator.platform === device));
 ...
 */
export const scrollElementIfOnLimit = (element) => {
...
const useIOSHackery = ({ onTouchStart = () => {}, onTouchEnd = () => {}, onScrollEnd = () => {}, ...props }) => {
  ...
  const attachTouchEventListeners = useCallback(() => {
    if (containerRef.current) {
      containerRef.current.addEventListener('touchstart', handleTouchStart, { capture: true });
